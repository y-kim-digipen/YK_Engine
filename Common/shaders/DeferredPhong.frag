...
{
    //Get geometry data
    vec3 normal = normalize( texture( normalBuffer, fs_in.vertexUV ).rgb );
    vec3 position = texture( viewPosBuffer, fs_in.vertexUV ).rgb;
    vec3 uvs = texture( uvBuffer, fs_in.vertexUV ).rgb;
    float depthFromEye = texture( depthBuffer, fs_in.vertexUV ).r;
